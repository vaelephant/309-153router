...
function SavingsTooltip({ active, payload, label, t }: {
  active?: boolean
  ...
  return (
    <div className="rounded-lg border border-border bg-popover px-3 py-2 shadow-lg text-xs">
      ...
      <p className="text-emerald-600">{t("analytics.savedLabel")}：¥{payload[0].value.toFixed(4)}</p>
    </div>
  )
}
...
export function AnalyticsSavings({ data, loading }: AnalyticsSavingsProps) {
    ...
    return (
      <Card className="bg-card border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium">{t("analytics.smartRoutingSavings")}</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const cards = [
    ...
    {
      label: t("analytics.routedRequests"),
            ...
            <div className="h-[200px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  />
                  <RechartsTooltip content={(props) => <SavingsTooltip {...props} t={t} />} />
                  <Area
                    type="monotone"
                    dataKey="saved"
                    stroke="#10b981"
